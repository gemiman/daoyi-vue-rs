  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    <el-form ref="formRef" v-loading="formLoading" :model="formData" :rules="formRules" label-width="100px">
      ...
      <el-form-item label="{{ col.columnComment }}" prop="{{ col.javaField }}">
        ...
        <el-input v-model="formData.{{ col.javaField }}" placeholder="请输入{{ col.columnComment }}" type="textarea"/>
        {% elif col.htmlType == 'datetime' %}
        <el-date-picker v-model="formData.{{ col.javaField }}" placeholder="选择{{ col.columnComment }}" type="datetime"
                        value-format="x"/>
        {% else %}
        <el-input v-model="formData.{{ col.javaField }}" placeholder="请输入{{ col.columnComment }}"/>
        {% endif %}
...
:
[{required: true, message: '{{ col.columnComment }}不能为空', trigger: 'blur'}],
{ % endif %
}
{%
  endfor %
