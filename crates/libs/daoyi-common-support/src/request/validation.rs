...
const USER_NAME_REGEX: LazyCell<Regex> =
    LazyCell::new(|| Regex::new(r"^[a-zA-Z0-9]+$").expect("Failed to compile username regex"));

pub fn is_valid_username(value: &str) -> Result<(), ValidationError> {
    if value.is_empty() || USER_NAME_REGEX.is_match(value) {
        Ok(())
    } else {
        Err(build_validation_error("用户账号由 数字、字母 组成"))
    }
}

pub fn is_mobile_phone(value: &str) -> Result<(), ValidationError> {
    if value.is_empty() || MOBILE_PHONE_REGEX.is_match(value) {
        Ok(())
    } else {
